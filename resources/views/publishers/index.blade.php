...
@section('content')
    <section class="bg-gray-50 container px-4 sm:px-6 mx-auto py-8 antialiased dark:bg-gray-900 md:py-16">
        <div class="mx-auto max-w-screen-xl px-4 2xl:px-0">
            {{-- search form --}}
            <div>
                <form class="max-w-md mx-auto" action="{{ route(name: 'gallery.publishers.search') }}" method="GET">
                    @csrf
                    ...
                    <div class="relative">
                        <input type="search" id="default-search"
                            ...
                            placeholder="ابحث عن ناشر" name="term" required />
                        <button type="submit"
                ...
                <div class="grid gap-4 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4">
                    @foreach ($publishers as $publisher)
                        <a href="{{ route('gallery.publishers.show', $publisher) }}"
                            ...
                            <span class="text-sm font-medium text-gray-900 dark:text-white">{{ $publisher->name }}</span>
            ...
            @else
                <div class="my-10 flex justify-center">
                    <h4 class="text-center text-2xl">لا يوجد ناشرين</h4>
                </div>
            @endif
