...
@section('content')
    <section class="bg-gray-50 container px-4 sm:px-6 mx-auto py-8 antialiased dark:bg-gray-900 md:py-16">
        <div class="mx-auto max-w-screen-xl px-4 2xl:px-0">
            {{-- search form --}}
            <div>
                <form class="max-w-md mx-auto" action="{{ route(name: 'gallery.authors.search') }}" method="GET">
                    @csrf
                    ...
                    <div class="relative">
                        <input type="search" id="default-search"
                            ...
                            placeholder="ابحث عن مؤلف" name="term" required />
                        <button type="submit"
                ...
                <div class="grid gap-4 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4">
                    @foreach ($authors as $author)
                        <a href="{{ route('gallery.authors.show', $author) }}"
                            ...
                            <span class="text-sm font-medium text-gray-900 dark:text-white">{{ $author->name }}</span>
            ...
            @else
                <div class="my-10 flex justify-center">
                    <h4 class="text-center text-2xl">لا يوجد مؤلفين</h4>
                </div>
            @endif
